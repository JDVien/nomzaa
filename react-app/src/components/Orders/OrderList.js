...
const OrderList = ({ user }) => {
  ...
  const single_order_num = order_group.map((product) => product.product);
  let next_order = [];
  let an_order = [];
  ...
  console.log(an_order, "an order ++++++++++++++");
  console.log(next_order, "next order --------------------");
  // const final_order = next_order[0].filter(item => item.purchased).map(product => product.product)
